...
def create_pipeline(cfg: dict, baseline_bic: float = None) -> "Pipeline":
    ...
    # PipelineSession intercepts processor.run() / estimator.fit() calls
    # during pipeline definition — prevents actual job submission and
    # allows ParameterString/ParameterInteger to pass through as-is.
    sess       = PipelineSession(boto_session=boto_sess)
    ...
    # ------------------------------------------------------------------
    # Processor — uses flat self-contained scripts (no src.* imports)
    # scripts/sm_preprocessing.py and scripts/sm_evaluation.py
    # ------------------------------------------------------------------
    ...
    step_preprocess = ProcessingStep(
        ...
        step_args=processor.run(
            code=os.path.join(scripts_dir, "sm_preprocessing.py"),
            inputs=[
                ProcessingInput(
                    source=p_products,
    ...
    step_eval = ProcessingStep(
        name="GMMEvaluation",
        step_args=processor.run(
            code=os.path.join(scripts_dir, "sm_evaluation.py"),
            inputs=[
                ProcessingInput(
    ...
    sm_model = Model(
        ...
        source_dir=proj_root,   # SDK zips proj_root and sets PYTHONPATH
    )
